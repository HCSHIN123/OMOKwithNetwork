...
void Scene::SetWindowSize(int _iWidth, int _iHeight)
{
	...
	m_vec2WindowStartPosition = { vec2ScreenStartPosition.x - (_iWidth / 2),
											  vec2ScreenStartPosition.y - (_iHeight / 2) };
	m_vec2WindowSize = { _iWidth,_iHeight };

	SetWindowPos(GameCore::GetInstance()->GethWnd(), nullptr, m_vec2WindowStartPosition.x, m_vec2WindowStartPosition.y,
